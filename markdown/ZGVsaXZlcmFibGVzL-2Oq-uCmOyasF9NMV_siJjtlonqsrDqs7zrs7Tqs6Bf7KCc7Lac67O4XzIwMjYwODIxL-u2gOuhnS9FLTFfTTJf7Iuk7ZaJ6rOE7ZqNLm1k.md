...
- **공적 기관 3곳은 별도 트랙으로 관리한다.** 
  - Track 1(수익화)·Track 2(VP 다지기)와 목표·속도·결정 구조를 다르게 가져간다
- **지자체와의 지속적인 관계 형성이 최우선 목표다.** 
  - 단기 계약 전환보다 의사결정권자와의 신뢰 쌓기를 앞세운다. 
  - 한 번 잡은 공공 컨택 포인트는 CRM을 넘어 비문 인식 등 다른 제품 라인의 판로로도 열린다
...
- **Santa Barbara ①**: 
  - 세 곳 중 규모가 압도적으로 크고, 
  - 페인포인트가 폼에서 직접 확인됐다(Chameleon + 화이트보드·스프레드시트 병용). 
- **HSWC Maryland ②**: 
  - 비영리라 조달이 단순하고 ED가 정식 임명 상태 
  — 사립이라 의사결정 경로가 셋 중 가장 짧다. 
  - 포스터 피크 300+마리 규모면 페인포인트 존재 가능성이 있으나 인터뷰 전까지는 가설 단계
- **TJO Springfield ③**: 
  - 파보 위기 재개장 직후라 지금은 안정화 모드일 가능성이 높다. 
  - 시립 기관 조달 복잡도 + ED 영구 임명 여부 미확인이 겹쳐 실질 전환까지 경로가 가장 길다. 
  - Cahillane 영구 임명이 확정되고 운영이 안정되면 2순위로 재평가한다
