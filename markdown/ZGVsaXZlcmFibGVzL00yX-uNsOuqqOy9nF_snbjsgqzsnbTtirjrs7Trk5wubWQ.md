...
| 판독 | 근거 · 함의 |
| --- | --- |
| **① 통증의 소재는 '입양 준비 판정'이 아니라 담당자 간 정보 단절 지점이다** | · 준비 완료 판정을 통증으로 진술한 응답자 **전건 없음.** 반복 확인된 축은 만기·지연·차기 조치·담당자<br>· SBCAS 실무진 8인 공통 — 한 개체의 정보를 여러 사람이 생성하나 단일 지점에 집약되지 않아 **입소 단계의 정보 누락이 입양 단계의 재조사로 전가된다**<br>· 문제는 "임보자의 응답 여부"에서 끝나지 않고 **"미응답 건의 재연락 담당자 지정"**까지 이어진다(Maybel's) |
| **② 경쟁 대상이 종이·수기 관리에서 기존 도구 조합으로 이동했다** | · 사용 도구 폐기 의사를 밝힌 응답자 **전건 없음.** 반면 상위 레이어 부착에는 전원 개방적<br>· Asheville 은 필요 기능을 이미 도구 6종으로 분산 해결 — 본인 표현 **"거미줄"**. `기존 도구 + Petify` 구성이 되면 도입 가치가 없다고 직접 우려<br>· **판정 기준은 도구 수의 실질 감축 여부.** LifeLine 도 동일 축 — Doobert 자체는 호평하나 PetPoint 와 연동되지 않아 담당자가 중간 단계를 수작업으로 보완한다 |
| **③ 제품 기본값은 조직 규모에 따라 달라진다** | · 대형은 **예외 건만 상신** — 임보자 업데이트 대부분이 직원 개입 불요(LifeLine, 임보자 1,500명 이상). 소형은 반대로 **추적 업무 자체의 제거**가 가치(Maybel's·Kzoo)<br>· **입력 단위와 기록 단위의 분리 필요** — 최종 기록이 개체별이어도 입력까지 개체별일 필요는 없다(PawSafe 한배 새끼)<br>· 자원봉사자 UX 가 도입 조건 — 고령·저숙련 임보자를 기술 사유로 배제할 수 없어 **전원 이전은 성공 기준으로 부적합.** 업무 배정도 개인보다 팀 단위가 운영에 부합(SBCAS) |
| **④ 첫 계약 논의가 개시됐고, 가격 저항선도 함께 확인됐다** | · **SBCAS 가 계약 초안 송부를 요청.** 카운티 계약 지연 시 연계 비영리기관 경유 경로, Chameleon 연동 불가 시 CSV 제공, `Karen Eval` 시트 공유까지 진전 — 제품 관심을 넘어선 첫 사례<br>· 반대편 저항선 — **$99/월은 전원 자원봉사 조직에 지속 불가능**(Maybel's). 규모별 가격 구간 설정이 필요하다<br>· 수요 발생 계기가 규제일 수 있다 — PawSafe 의 소프트웨어 탐색 직접 계기는 **뉴욕주 임보 기록 의무 강화**(법적 실체 미검증) |

**미결 — 알파 MVP 설계 대기 항목**

- **적용 범위 확정** — 셸터 메디신 체크리스트(LTBHS)·의료 워크플로우(SBCAS)·임보 커뮤니케이션(Kzoo·CAP)을 단일 제품으로 볼 것인가. SBCAS 의료팀은 검사·재진 종료 전 조기 이동과 의료 정보 미전달을 **별도 사안**으로 제기
- **규모별 기본값 분기** — 전건 검토가 부담으로 전환되는 임보자 규모, 그리고 고정 주기(일·주 단위) 대비 이벤트 기반(백신 만기·투약 종료) 발송의 선택
- **배정·입력 단위** — 개인 배정 전제를 팀 단위(SBCAS)로 전환할 것인가. 사용자별 과금 구조는 공동 운영자 4인이 동일 권한인 조직(Maybel's)과 상충
- **영상 업로드 경로** — Maybel's 요구 사항에 사진과 영상이 함께 포함되나 데모는 사진만 구현. 초기 통증의 절반이 미검증

**검증 공백 — 판독 확정에 필요한 항목**

- **가격·예산 소유자·결재 절차** — 전 콜 공통 항목이나 실제 질의 사례가 없다
- **Shelterluv 연동 범위**(Asheville) — `상태 변화 감지 → 자동 체크인 → 직원 승인 → 기록 반영`의 왕복 구조. 단순 데이터 조회 수준으로는 도입 근거가 성립하지 않는다
- **CAP 전결 한도 및 Jared 의 임보 프로세스 소유 여부** — "부서 예산을 통한 조기 도입" 판단 전체가 이 전제에 의존
...
| 10 | B | **LifeLine Animal Project**<br>Daisy Navin · Program Manager(셸터 3개소)<br>임보자 1,500명+ · 셸터당 개 ~500두 | <span class="pain-badge high">강함</span><br>본인 | · 정보와 흐름이 **PetPoint·Doobert·Gmail·Google Voice 로 분산**되고 상호 자동 연계가 없음<br>· 이메일로 세 가지를 물으면 임보자가 **한 가지만 답함**<br>· 1개월 이상 미응답은 공식 처리 절차로 이어짐 — **지점당 연 20~50건** | PetPoint<br>Doobert Connect<br>Gmail · Google Voice |
...
| 12 | B | **Asheville Humane Society**<br>Mallory Uzel · 임보 프로그램 총괄<br>동시 임보 180~200두 · 임보팀 4인 | <span class="pain-badge high">강함</span><br>본인 | · **필요 기능이 없는 게 아니라 복수 소프트웨어에 분산돼 있다** — 본인 표현으로 현 구조가 "거미줄"<br>· 가장 불필요한 업무로 **복수 시스템 간 전환 반복 자체**를 지목<br>· 도구 6종이 각각 제 역할은 하나 서로 연결되지 않음 | Shelterluv · Maddie's Pet Assistant<br>Google Forms · MailerLite<br>Quo · Setmore |
...
| 10 | B | **LifeLine Animal Project**<br>Daisy Navin · Program Manager<br>임보자 1,500명+ | <span class="fit-badge good">높음</span><br>add-on | · 구조화 양식이 **"질문했는데 일부만 답하는 문제"를 줄인다**고 본인이 직접 평가<br>· 시스템 안에서 문자를 보내는 점을 강하게 선호 — 현재 Google Voice 가 부담<br>· **PetPoint 교체 의향 없음 명시** — 공백만 보완하는 위치가 현실적<br>· **단 전건 검토 구조는 이 규모에서 역효과** — 대부분의 업데이트가 직원 개입 불요 |
