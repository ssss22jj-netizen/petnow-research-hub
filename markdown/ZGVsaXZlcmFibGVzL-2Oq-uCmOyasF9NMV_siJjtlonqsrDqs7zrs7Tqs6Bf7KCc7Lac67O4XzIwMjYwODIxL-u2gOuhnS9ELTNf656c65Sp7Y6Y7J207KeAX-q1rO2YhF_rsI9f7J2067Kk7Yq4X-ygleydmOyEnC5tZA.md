...
> 최종 개정: 2026-08-20 (별첨 B 데모 레퍼러 트래킹 기획 첨부). 직전 개정: 2026-08-14 (별첨 A 개발용 사이트 구현 스펙 추가)
...
## 별첨 B. 데모 레퍼러 트래킹 기획 (2026-08-19 신설)

> 결론: 인터뷰이에게 발송하는 데모 링크에 `ref` 파라미터를 부여해 리드 시트 행과 Mixpanel 이벤트를 별도 매핑 없이 연결. 상세 스펙은 아래 첨부 문서로 관리하며, 본 정의서에는 연결 관계와 개정 사항만 기재.
...
| --- | --- |
| 문서명 | 데모 레퍼러 트래킹 기획 |
| 주소 | https://claude.ai/code/artifact/2622d134-2947-4f63-b889-a6a14ce8edfd |
| 작성 | 2026-08-19 |
| 수록 범위 | URL 파라미터 설계, Mixpanel 이벤트 택소노미(`Demo` 접두사), 리드 시트 추가 컬럼 4종, 방문·공유 플로우 3종 |

### B.2 본 정의서와의 관계

| 구분 | 랜딩 (본문) | 데모 (별첨 B) |
| --- | --- | --- |
| 이벤트 접두사 | 없음 (`Landing Viewed`·`Lead Submitted` 등) | `Demo` |
| Mixpanel 프로젝트 | `Petify for shelters_Live` | 동일 |
| 식별자 | `lead_id` | `lead_id` 앞 8자리를 `ref` 파라미터로 사용 |
| 시트 반영 | 리드 행 신규 생성 | 기존 리드 행에 카운터 컬럼 누적 |
