...
소재 B가 겨냥한 **임보자 체크인 회수**가 셸터 위탁 담당자의 어려움으로 지목됐고, 응답자가 이를 데모 신청 이유로 직접 밝혔다. 다만 응답자는 그 셸터의 자원봉사자이고 셸터 소프트웨어 접근권이 없어, 이 콜은 도입 판단이 아니라 간접 증언으로 다룬다. 후속으로 카운티 실무자 소개를 수락했다.
...
- 셸터로 돌려보내지 않고 집에서 입양 희망자를 만나는 방식을 선호. 입양이 확정되면 중성화 예약을 잡는다
- 입양 대신 **주(州) 밖 다른 셸터로 이송**되는 경우도 있다
- 셸터에 상근 수의사가 있어(디렉터 겸임) 인테이크 시 검진과 투약 준비를 맡는다. 임보자는 그 뒤 벼룩 처리와 행동 교정(발톱깎이·청소기·사람 접촉 적응)을 담당한다
...
- **응답자는 이것을 데모를 신청한 이유로 직접 밝혔다**
- 응답자 본인의 임보 업무에서는 Airtable로 처리하고 있으나, **개체 유입이 몰리거나 바쁠 때는 그렇지 않다고 조건을 달았다** — `air table's fine if we haven't had an influx of cats or busy`

### 3.1 함께 확인된 통증 — 백신 만기 조회 `22:38`

"최근에 추적이 어려워진 순간"을 묻자 첫 답으로 나온 것이 백신이다. 검증 대상과 같은 **임보 중 기록 관리** 축이다.
...
- 셸터 위탁 담당자는 임보 중인 개체의 접종 예정일을 **한 번에 보기 어렵다**
- 응답자 본인도 접종일을 놓친 뒤 대상을 찾을 때 수작업이 더 든다고 말했다. 불가능한 것은 아니지만 손이 간다는 것이다
...
| 현행 대비 `32:10` | 모바일 대응을 최대 차이로 꼽았다. 단 Airtable의 구조적 한계가 아니라 **본인이 인터페이스를 아직 만들지 않았기 때문**이다 — `mainly because i haven't built some of the interfaces... i could fix that i just haven't done it` |
| 당장 전환 `37:15` | **아니오.** 보완재로 병행하겠다 — `it's close but i would supplement` |
| 전환을 막은 갭 `33:43` | ①3개월 미만 개체의 **일일 체중 추적** ②개체별로 다른 항목을 넣을 **커스터마이즈 필드** |
| 지불 의향 | 미확인 |
...
- **물품 추적은 응답자가 실제로 겪고 있는 문제로 보인다** — 부족분을 자기 돈으로 사고 있고 [검증됨·L0], "하나만 고칠 수 있다면"이라는 질문에 이것을 답으로 골랐다 [검증됨·L0]. 셸터에 이미 산식까지 제안해둔 상태이므로 [검증됨·L0], 여러 번 생각해온 사안으로 읽힌다 [추론·L2]
...
| 물품 요청 | `maybe a supplies section like hey this specific dog or cat needs diapers so do you need more diapers` — 체크인 폼 항목으로 붙이는 형태 |
| 백신 만기 조회 | 위탁 중 개체의 백신 예정일을 한 번에 보는 화면. 3.1 참조 |

## 7. 남은 것

1. **셸터 도구명 확인** — 응답자가 이메일로 회신 예정 (`08:52`에 약속)
2. **소개 회수** — 카운티 실무자 소개를 이메일로 요청 (콜에서 조니가 후속 메일 약속)
3. **제품 업데이트 후속 연락** — 콜에서 조니가 연락 유지에 동의를 받았다 (`41:02`)
...
- 계산 자체는 어렵지 않다. 본인이 셸터에 산식을 이미 제안해뒀다 — `let's say one wet food can a day times that and that should be your weekly input`
