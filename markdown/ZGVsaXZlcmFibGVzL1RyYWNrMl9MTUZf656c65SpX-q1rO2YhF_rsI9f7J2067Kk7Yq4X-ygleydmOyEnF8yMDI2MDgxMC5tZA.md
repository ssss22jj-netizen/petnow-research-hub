...
| `Lead Submitted` | Google Apps Script의 저장 성공 응답 수신 | `landing_variant`, `event_id`, `lead_intent`, `annual_adoptions`, `active_fosters`, `animal_origin`, `current_tools`, `organization_type`, `respondent_role`, `animal_record_system` |
...
- Mixpanel 리드 속성: 조직명·이름·이메일·허니팟을 제외한 폼 필드를 각각 수집. `organization_type`은 조직 유형, `respondent_role`은 응답자 직무, `animal_record_system`은 현재 동물 기록 관리 도구를 의미
