...
## 3. Track 2 — 또다른 페인포인트 축 검증 (제품 VP 다지기)
...
- ICE 점수 산출 근거와 소재 축소 경위는 [Track 2 LMF 가설 ICE 재스코어링](https://ssss22jj-netizen.github.io/petnow-research-hub/#/doc/ZGVsaXZlcmFibGVzL1RyYWNrMl9MTUZf6rCA7ISkX0lDRV_snqzsiqTsvZTslrTrp4FfMjAyNjA4MDkubWQ) 별첨 참조
...
## 4. 공공기관 별도 트랙 — 지자체 관계 구축

### 4.1 트랙 의의

- **공적 기관 3곳은 별도 트랙으로 관리한다.** Track 1(수익화)·Track 2(VP 다지기)와 목표·속도·결정 구조가 달라 동일 흐름에 넣으면 리소스 배분이 왜곡된다
- **지자체와의 지속적인 관계 형성이 최우선 목표다.** 단기 계약 전환보다 의사결정권자와의 신뢰 쌓기를 앞세운다. 한 번 잡은 공공 컨택 포인트는 CRM을 넘어 비문 인식 등 다른 제품 라인의 판로로도 열린다
- **산타바바라 카운티 인터뷰에서 인사이트를 먼저 확보하고**, 같은 인사이트가 나머지 두 곳(HSWC Maryland, TJO Springfield)에도 성립하는지 순차 확인한다
- **제품팀 리소스 배분은 세 곳 중 산타바바라를 우선한다** (프로토타입·개발 검토 등 요청이 생길 경우)

### 4.2 리드 현황 및 우선순위

| | Santa Barbara (Aguilar) | HSWC Maryland (Colin Berry) | TJO Springfield (Cahillane) |
|---|---|---|---|
| **규모** | 인테이크 6,475 / 입양 ~3,000 / 포스터 배치 6,204건 / 쉘터 3곳 | 인테이크 미확인 / 포스터 피크 300+마리 | 인테이크 ~4,000 (5년 전 대비 +60%) |
| **연예산** | ~$6M | $3.89M (2024) | 시 예산 편입, 규모 미확인 |
| **의사결정권자** | Director Aguilar, 정식·재임 4년차 | ED Colin Berry, 정식. 기금조달 출신, 성과 지표 언어 유효 | ED Cahillane — 2026 기사 "interim" 표기 탈락, 영구 임명 여부 확인 필요 |
| **조직 형태** | 카운티 정부 산하 | 비영리 (카운티 계약) — **조달 가장 단순** | 시립 (Springfield 市 산하) — 시 조달 라인 |
| **기존 도구** | Chameleon + 화이트보드·스프레드시트 (폼 직접 기재) | BetterImpact(자원봉사)만 확인 / 쉘터 SW 미확인 | 미확인 (현장 서식 기반 기록 존재) |
| **현재 상황** | 예산 삭감 압박 / 8/20 데모 콜 확정 | 안정적 | 파보 재개장 한 달 (7/24 이후), 수습 중 |
| **우선순위** | **① 최우선** | **② 2순위** | **③ 3순위** |

**판단 근거:**

- **Santa Barbara ①**: 세 곳 중 규모가 압도적으로 크고, 페인포인트가 폼에서 직접 확인됐다(Chameleon + 화이트보드·스프레드시트 병용). 콜이 이미 잡혀 있어 준비 자원이 집중되어야 하는 시점
- **HSWC Maryland ②**: 비영리라 조달이 단순하고 ED가 정식 임명 상태 — 의사결정 경로가 셋 중 가장 짧다. 포스터 피크 300+마리 규모면 페인포인트 존재 가능성이 있으나 인터뷰 전까지는 가설 단계
- **TJO Springfield ③**: 파보 위기 재개장 직후라 지금은 안정화 모드일 가능성이 높다. 시립 기관 조달 복잡도 + ED 영구 임명 여부 미확인이 겹쳐 실질 전환까지 경로가 가장 길다. Cahillane 영구 임명이 확정되고 운영이 안정되면 2순위로 재평가한다

### 4.3 사전조사 문서

| 기관 | 사전조사 | 데모 전략 |
|---|---|---|
| Santa Barbara County Animal Services | [사전조사 →](../analysis/SantaBarbara_카운티_사전조사_20260817.md) | [데모 미팅 전략 →](M2_산타바바라_데모미팅_전략_20260818.md) |
| HSWC Maryland | [사전조사 →](../analysis/HSWC_Maryland_사전조사_20260819.md) | 콜 일정 미확정 |
| TJO Springfield | [사전조사 →](../analysis/TJO_Springfield_사전조사_20260819.md) | 콜 일정 미확정 |
...
| v1.3 | 2026-08-19 | §4 공공기관 별도 트랙 신설. 3개 리드(Santa Barbara·HSWC Maryland·TJO Springfield) 현황·우선순위·사전조사 연결 추가. 기존 §4 예산·일정 → §5로 이동 |
