...
const BASE_ANIMALS = [
  { id:'pepper', recordId:'1090', name:'Pepper', img:'assets/pepper.png', meta:'Mixed breed · Female · 2 years', queue:'new', stage:'New intake', readiness:'neutral', label:'New intake', blocker:'Foster placement', next:'Place in foster', owner:'Alex Rivera', updated:'Today' },
  { id:'nala', recordId:'1091', name:'Nala', img:'assets/nala.png', meta:'Terrier mix · Female · 1 year', queue:'new', stage:'New intake', readiness:'neutral', label:'New intake', blocker:'Foster placement', next:'Place in foster', owner:'Sam Chen', updated:'Today' },
  { id:'scout', recordId:'1092', name:'Scout', img:'assets/scout.png', meta:'Jindo mix · Male · 3 years', queue:'new', stage:'New intake', readiness:'neutral', label:'New intake', blocker:'Foster placement', next:'Place in foster', owner:'Morgan Kim', updated:'Yesterday' },
  { id:'olive', name:'Olive', img:'assets/olive.png', meta:'Terrier mix · Female · 5 years', queue:'care', stage:'In foster', readiness:'neutral', label:'In care', blocker:'None', next:'Open record', owner:'Sam Chen', updated:'Today' },
...
function translateText(value) {
  ...
  if(!translated) {
    const rules=[
      ...
      [/^(All|New intake|In care|Ready|Ready to publish|Blocked|Needs review|Published|Overdue|Sent|In progress|Changes requested|Approved|Active|Available|Inactive|Updates due|Responded) (\d+)$/,(_,a,b)=>`${KO_UI[a]||a} ${b}`],
      ...
      [/^(\d+) of (\d+) foster profiles$/,(_,a,b)=>`임보자 프로필 ${b}명 중 ${a}명`],
      [/^(.+) response rate$/,(_,a)=>`응답률 ${a}`],
      [/^Open (.+)’s record$/,(_,a)=>`${a} 레코드 열기`],
      [/^(\d+) of 5 sections complete$/,(_,a)=>`5개 항목 중 ${a}개 완료`],
      [/^(\d+) of 5 logged$/,(_,a)=>`5개 중 ${a}개 기록`],
      [/^(\d+) of 5 logged · medication due$/,(_,a)=>`5개 중 ${a}개 기록 · 투약 예정`],
      [/^(\d+) of 5 logged · Saved just now$/,(_,a)=>`5개 중 ${a}개 기록 · 방금 저장`],
      [/^(\d+) min ago$/,(_,a)=>`${a}분 전`],
...
function animals() {
  ...
  list.filter(animal=>animal.queue==='new'&&state.placements[animal.id]).forEach(animal=>{
    Object.assign(animal, {queue:'care', stage:'In foster', readiness:'neutral', label:'In care', blocker:'None', next:'Request first update', updated:'Just now'});
  });
...
function animalQueueTotals() {
  ...
  const baseNewAssigned=BASE_ANIMALS.filter(animal=>animal.queue==='new'&&state.placements[animal.id]).length;
  const cocoAssigned=Boolean(state.placements.coco);
  return {
    new:3-baseNewAssigned+(state.cocoCreated&&!cocoAssigned?1:0),
    care:28+baseNewAssigned+(cocoAssigned?1:0)+(!state.fosterSubmitted&&!miloIsReady&&!miloIsPublished?1:0),
...
function animalsView() {
  ...
  const placeholderCount=state.search?0:Math.max(0,targetCount-list.length);
...
function updateQueueTotals() {
  const waitingForResponse=state.requestSent&&!state.fosterSubmitted;
  const dueToday=!state.requestSent&&!state.fosterSubmitted;
  const needsReview=state.fosterSubmitted&&!state.updateApproved;
  return {
    overdue:4,
    sent:7+(waitingForResponse?1:0),
    progress:3,
    review:needsReview?1:0,
    due:dueToday?1:0,
    requested:1,
    approved:1+(state.updateApproved?1:0)
  };
}

function updateQueueLabel(queue) {
  return {overdue:'Overdue',sent:'Sent',progress:'In progress',review:'Needs review',due:'Due today',requested:'Changes requested',approved:'Approved'}[queue]||'All';
}

function placeholderUpdateCard(queue,index) {
  return `<div class="submission placeholder-submission" aria-label="Additional ${updateQueueLabel(queue).toLowerCase()} update record"><span class="dummy-update-avatar"></span><span><i class="dummy-update-line wide"></i><i class="dummy-update-line medium"></i><i class="dummy-update-line short"></i></span></div>`;
}

function updateSubmissionCard(item,selectedId) {
  if(item.placeholder) return placeholderUpdateCard(item.filter,item.index);
...
function updatesView() {
  ...
  const currentSubmissions = UPDATE_SUBMISSIONS.map(x=>{
    ...
    if(!state.requestSent&&!state.fosterSubmitted) return {...x,status:'Due today',statusType:'failed',filter:'due',flag:'Request not sent',time:'Due today'};
    if(state.fosterSubmittedNow) return {...x,time:'Just now'};
    return x;
  });
  const totals=updateQueueTotals();
  ...
  const placeholderSubmissions=visibleQueues.flatMap(queue=>{
    const realCount=currentSubmissions.filter(item=>item.filter===queue).length;
    const missing=Math.max(0,(totals[queue]||0)-realCount);
    return Array.from({length:missing},(_,index)=>({placeholder:true,filter:queue,index}));
  });
  const submissions=[...realSubmissions,...placeholderSubmissions];
  ...
  return `<section class="content">
    ...
    <div class="update-context"><span>UPDATE WORKFLOW STATUS</span><small>Each request or submission appears in one status queue. Select a card to open the matching list.</small></div>
    ...
    <section class="surface inbox">
      <aside class="inbox-sidebar"><div class="inbox-tabs">${[['all',`All ${allCount}`],['review',`Needs review ${totals.review}`],['requested',`Changes requested ${totals.requested}`],['approved',`Approved ${totals.approved}`]].map(([id,l])=>`<button class="${state.updateFilter===id?'active':''}" data-update-filter="${id}">${l}</button>`).join('')}</div><label class="inbox-search">⌕ <input placeholder="Search updates"></label><div class="submission-count"><b>${submissions.length}</b> ${state.updateFilter==='all'?'updates':updateQueueLabel(state.updateFilter).toLowerCase()}</div><div class="submission-list">${submissions.map(s=>updateSubmissionCard(s,item?.id)).join('')}</div></aside>
      ${reviewPanel(item)}
    </section>
  </section>`;
}
...
function reviewPanel(item) {
  if(!item) return `<article class="review-panel empty-review"><div class="empty"><span>✓</span><h3>No updates in this status</h3><p>Choose another status to view its requests and submissions.</p></div></article>`;
...
function fostersView() {
  ...
  const totals={active:18+placementCount,available:12-placementCount,inactive:1,due:7,responded:29};
  totals.all=totals.active+totals.available+totals.inactive;
  ...
  const targetCount=state.fosterFilter==='all'?totals.all:totals[state.fosterFilter];
  const missing=Math.max(0,targetCount-realRows.length);
  const visibleRows=[...realRows,...Array.from({length:missing},(_,index)=>({placeholder:true,index}))];
  const footerText=`${targetCount} of ${targetCount} foster profiles`;
  return `<section class="content">
    ${pageHeader('FOSTER NETWORK','Fosters','Keep placements, contact details, daily care, and update schedules connected.',`<button class="secondary-button" data-action="open-daily-care">Open foster mobile view</button><button class="primary-button" data-action="new-placement">＋ New placement</button>`)}
    <div class="update-context"><span>FOSTER NETWORK STATUS</span><small>Status tabs are mutually exclusive. Due and response metrics are cross-status operational filters.</small></div>
    <section class="surface care-program"><div class="care-program-head"><div><p class="kicker">CARE CADENCE</p><h2>Daily care and weekly updates</h2><p>Routine care is logged each day. Health, behavior, and adoption-ready media are reviewed in the weekly check-in.</p></div><button class="secondary-button" data-action="open-daily-care">Preview Jamie’s mobile care</button></div><div class="care-cadence-grid"><button data-action="open-daily-care"><span class="care-icon">☀</span><span><small>TODAY · MILO</small><b>Daily care</b><em>${Object.values(state.dailyTasks).filter(Boolean).length} of 5 logged · medication due</em></span><i>›</i></button><button data-action="open-weekly-checkin"><span class="care-icon weekly">↻</span><span><small>THIS WEEK · MILO</small><b>Weekly check-in</b><em>Behavior, health changes, and photos</em></span><i>›</i></button><div><span class="care-icon exception">!</span><span><small>EXCEPTION RULE</small><b>Only issues need staff review</b><em>Missed medication or concerns alert Alex</em></span></div></div></section>
    <div class="metric-grid four foster-metrics">${[['Active placements',totals.active,'2 ending this month','active'],['Available fosters',totals.available,'4 match current needs','available'],['Updates due',totals.due,'4 overdue','due'],['Response rate','94%','29 of 31 responded','responded']].map(([a,b,c,f])=>`<button class="metric-card ${state.fosterFilter===f?'active':''}" data-foster-filter="${f}"><span><b>${a}</b>${formatCount(b)}<small>${c}</small></span><i>›</i></button>`).join('')}</div>
    <section class="surface table-surface"><div class="table-toolbar"><div class="filter-tabs">${[['all',`All ${totals.all}`],['active',`Active ${totals.active}`],['available',`Available ${totals.available}`],['inactive',`Inactive ${totals.inactive}`]].map(([id,label])=>`<button class="${state.fosterFilter===id?'active':''}" data-foster-filter="${id}">${label}</button>`).join('')}</div><div class="toolbar-actions"><button class="filter-button" data-action="foster-filter">☷ Filters ${state.fosterFilter==='all'?'':'<span>1</span>'}</button><button class="filter-button" data-action="foster-sort">↕ Sort</button></div></div><div class="table-wrap"><table><thead><tr><th>Foster</th><th>Current placement</th><th>Status</th><th>Update schedule</th><th>Response rate</th><th></th></tr></thead><tbody>${visibleRows.map(fosterRow).join('')}</tbody></table></div><div class="table-footer"><span>${footerText}</span><div><button data-action="prev-page">‹</button><b>1</b><button data-action="next-page">›</button></div></div></section>
  </section>`;
}

function fosterRow(row) {
  if(row.placeholder) return `<tr class="placeholder-row" aria-label="Additional foster profile"><td><span class="owner large"><i class="dummy-foster-avatar"></i><span><i class="skeleton-line wide"></i><i class="skeleton-line medium"></i></span></span></td><td><i class="skeleton-line medium"></i></td><td><span class="skeleton-pill"></span></td><td><i class="skeleton-line wide"></i></td><td><i class="skeleton-line short"></i></td><td><span class="skeleton-dot">•••</span></td></tr>`;
...
function handleAction(action, el) {
  ...
  switch(action) {
    ...
    case 'open-form-from-preview': state.drawer=null;state.modal=null;state.fosterFormMode='weekly';state.fosterSubmitted=false;state.fosterSubmittedNow=false;state.uploads=0;state.view='fosterform';render();break;
    ...
    case 'preview-foster-form': state.drawer=null;state.fosterFormMode='weekly';state.fosterSubmitted=false;state.fosterSubmittedNow=false;state.uploads=0;state.view='fosterform';render();break;
    case 'open-weekly-checkin': state.fosterFormMode='weekly';state.fosterSubmitted=false;state.fosterSubmittedNow=false;state.uploads=0;state.view='fosterform';render();break;
