...
const TOUR_SETUP = {
  ...
  b1: {view:'updates', requestSent:false, fosterSubmitted:false, updateApproved:false, miloReady:false},
...
function updatesView() {
  ...
  return `<section class="content">
    ...
    ${state.fosterSubmittedNow&&state.formChoice==='same'?`<section class="surface activity-card"><div class="section-head"><div><p class="kicker">RECENT ACTIVITY</p><h2>Recent activity</h2></div></div><div class="activity-list">${activityItem('assets/dog1.png','Milo · No changes reported · logged automatically','No staff review requested','Just now')}</div></section>`:''}
  </section>`;
}
...
function fostersView() {
  ...
  return `<section class="content">
    ...
    <section class="surface care-program"><div class="care-program-head"><div><p class="kicker">CARE & CHECK-IN SCHEDULE</p><h2>Daily care and weekly check-ins</h2><p>Routine care is logged each day. Health, behavior, and adoption photos are reviewed in the weekly check-in.</p></div><button class="secondary-button" data-action="open-daily-care">Preview Jamie’s foster view</button></div><div class="care-cadence-grid"><button data-action="open-daily-care"><span class="care-icon">☀</span><span><small>TODAY · MILO</small><b>Daily care</b><em>${Object.values(state.dailyTasks).filter(Boolean).length} of 5 care tasks logged · medication due</em></span><i>›</i></button><button data-action="open-weekly-checkin"><span class="care-icon weekly">↻</span><span><small>THIS WEEK · MILO</small><b>Weekly check-in</b><em>Behavior updates, health changes, and photos</em></span><i>›</i></button><div><span class="care-icon exception">!</span><span><small>NEEDS STAFF ATTENTION</small><b>Only issues need staff review</b><em>Alex is alerted to missed medications or reported concerns.</em></span></div></div></section>
...
function handleAction(action, el) {
  ...
  switch(action) {
    ...
    case 'tutorial-next': tutorialHistory.push(JSON.parse(JSON.stringify(state)));state.tutorialStep=Math.min(tutorialSteps().length-1,state.tutorialStep+1);render();break;
