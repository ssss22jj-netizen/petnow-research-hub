...
const NAVIGATION_KEYS = ['view','animalId','detailTab','animalFilter','updateFilter','fosterFilter','publishFilter','settingsTab','selectedUpdate','selectedFoster','search'];
...
const BASE_ANIMALS = [
  { id:'pepper', recordId:'1090', name:'Pepper', img:'assets/dog4.png', meta:'Mixed breed · Female · 2 years', queue:'new', stage:'New intake', readiness:'neutral', label:'New intake', blocker:'Foster placement', next:'Place in foster', owner:'Alex Rivera', updated:'Today' },
  { id:'nala', recordId:'1091', name:'Nala', img:'assets/luna-play.jpg', meta:'Terrier mix · Female · 1 year', queue:'new', stage:'New intake', readiness:'neutral', label:'New intake', blocker:'Foster placement', next:'Place in foster', owner:'Sam Chen', updated:'Today' },
  { id:'scout', recordId:'1092', name:'Scout', img:'assets/buddy-walk.jpg', meta:'Jindo mix · Male · 3 years', queue:'new', stage:'New intake', readiness:'neutral', label:'New intake', blocker:'Foster placement', next:'Place in foster', owner:'Morgan Kim', updated:'Yesterday' },
...
const KO_UI = {
  ...
  'FOSTER NETWORK':'임보자 네트워크','Keep placements, contact details, and update schedules connected.':'배치, 연락처, 업데이트 일정을 연결해 관리합니다.','Invite foster':'임보자 초대','New placement':'새 임보 배치','Active placements':'진행 중인 임보','Available fosters':'배치 가능한 임보자','Updates due':'업데이트 마감','Response rate':'응답률','Last 30 days':'최근 30일','FOSTER NETWORK STATUS':'임보자 네트워크 상태','Select a card to open the matching foster list. Response rate shows 29 of 31 people.':'카드를 누르면 해당 임보자 목록이 열립니다. 응답률은 31명 중 29명 응답을 기준으로 합니다.','29 of 31 responded':'31명 중 29명 응답','Responded':'응답 완료','Current placement':'현재 배치','Status':'상태','Update schedule':'업데이트 일정','Active':'활성','Available':'사용 가능','No placement':'배치 없음','Weekly':'매주','Every 2 weeks':'2주마다','Availability':'가능 여부','Sort':'정렬',
...
function translateText(value) {
  ...
  if(!translated) {
    const rules=[
      ...
      [/^(All|New intake|In care|Ready|Ready to publish|Blocked|Needs review|Published|Overdue|Sent|In progress|Changes requested|Approved|Active|Available|Updates due|Responded) (\d+)$/,(_,a,b)=>`${KO_UI[a]||a} ${b}`],
...
const UPDATE_SUBMISSIONS = [
  ...
  {
    id:'rocky', name:'Rocky', img:'assets/dog4.png', foster:'Casey Park', time:'5 days late',
  ...
  {
    id:'maple', name:'Maple', img:'assets/dog2.png', foster:'Jordan Lee', time:'2 hr ago',
  ...
  {
    id:'nori', name:'Nori', img:'assets/dog3.png', foster:'Taylor Reed', time:'38 min ago',
...
function animals() {
  ...
  if (state.cocoCreated) list.unshift({
    id:'coco', recordId:'1093', name:'Coco', img:'assets/dog4.png', meta:'Mixed breed · Female · 2 years',
...
function updatesView() {
  ...
  const submissions=visibleQueues.flatMap(queue=>{
    const real=currentSubmissions.filter(item=>item.filter===queue);
    const missing=Math.max(0,(totals[queue]||0)-real.length);
    return [...real,...Array.from({length:missing},(_,index)=>({placeholder:true,filter:queue,index}))];
  });
  const selectedIndex=submissions.findIndex(entry=>!entry.placeholder&&entry.id===selected);
  if(selectedIndex>0) submissions.unshift(...submissions.splice(selectedIndex,1));
  const realSubmissions=submissions.filter(item=>!item.placeholder);
...
function fostersView() {
  ...
  const missing=state.fosterFilter==='all'?0:Math.max(0,targetCount-realRows.length);
  const visibleRows=[...realRows,...Array.from({length:missing},(_,index)=>({placeholder:true,index}))];
  const footerText=state.fosterFilter==='all'?`${rows.length} named demo profiles · 31 foster profiles total`:`${targetCount} of ${targetCount} foster profiles`;
  return `<section class="content">
    ...
    <div class="update-context"><span>FOSTER NETWORK STATUS</span><small>Select a card to open the matching foster list. Response rate shows 29 of 31 people.</small></div>
